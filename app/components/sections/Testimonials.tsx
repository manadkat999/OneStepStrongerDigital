  ...
  return (
    <div className="flex gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <svg key={i} className="w-4 h-4 text-[#1C1917]" fill="currentColor" viewBox="0 0 20 20">
...
export default function Testimonials() {
  ...
  return (
    <Section id="results" className="bg-[#F5F5F4]">
      <div className="absolute inset-0 grid-bg opacity-40" />
      <div className="absolute top-1/2 right-1/4 w-[500px] h-[500px] rounded-full bg-stone-400/10 blur-[120px] pointer-events-none" />
      ...
      <Container className="relative z-10">
        ...
        <div className="max-w-3xl mx-auto mb-10">
          <div className="card-glow rounded-3xl p-10 md:p-12 text-center relative overflow-hidden">
            ...
            <span className="inline-block bg-[#EFEEEC] border border-[#E7E5E4] rounded-full px-3 py-1 text-[#57534E] text-xs mb-5">
              {t.industry}
            </span>

            <blockquote key={active} className="text-xl md:text-2xl font-medium text-[#1C1917] leading-relaxed mb-8 animate-fade-up">
              &ldquo;{t.quote}&rdquo;
            </blockquote>
            ...
            <div className="flex items-center justify-center gap-4 flex-wrap">
              ...
              <div className="text-left">
                ...
                <p className="text-[#57534E] text-xs">{t.role}</p>
              </div>
              <div className="ml-3 pl-4 border-l border-stone-300">
                <p className="text-2xl font-light" style={{ color: t.color }}>{t.result}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-center gap-2 mt-5">
            {TESTIMONIALS.map((_, i) => (
              <button
                key={i}
                onClick={() => restart(i)}
                className={cn("h-1.5 rounded-full transition-all duration-300", i === active ? "w-7 bg-[#1C1917]" : "w-2 bg-stone-300 hover:bg-stone-400")}
              />
            ))}
          </div>
        </div>
          ...
          {TESTIMONIALS.map((item, i) => (
            <button
              ...
              className={cn(
                ...
                i === active && "border-[#1C1917]/40 shadow-lg shadow-stone-900/10"
              )}
            >
              ...
              <p className="text-[#1C1917] text-xs font-semibold leading-tight">{item.name.split(" ")[0]}</p>
              <p className="text-[#57534E] text-xs mt-0.5">{item.industry}</p>
