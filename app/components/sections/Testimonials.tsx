...
export default function Testimonials() {
  ...
  return (
    <Section id="results" className="bg-[#080808]">
          ...
          <div className="flex justify-center gap-2 mt-5">
            {TESTIMONIALS.map((_, i) => (
              <button
                key={i}
                onClick={() => restart(i)}
                className={cn("h-1.5 rounded-full transition-all duration-300", i === active ? "w-7 bg-orange-500" : "w-2 bg-gray-700 hover:bg-gray-500")}
              />
            ))}
          </div>
        </div>
          ...
          {TESTIMONIALS.map((item, i) => (
            <button
              ...
              className={cn(
                ...
                i === active && "border-orange-500/40 shadow-lg shadow-orange-500/8"
              )}
            >
              ...
              <p className="text-gray-500 text-xs mt-0.5">{item.industry}</p>
